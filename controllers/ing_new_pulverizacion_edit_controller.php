...
try {
    ...
    if ($method === 'GET') {
        switch ($action) {
            ...
            case 'solicitud':
                ...
                $det = $model->detalleSolicitud($id);
                if (!$det || !is_array($det)) {
                    $resp([], false, 'No se encontró la solicitud');
                    break;
                }

                // Devolver tal cual lo arma el modelo (contrato que consume el front)
                $resp($det);
                break;
        }
        exit;
    ...
    if ($method === 'POST') {
        ...
        if ($payload['forma_pago_id'] === 6 && empty($payload['coop_descuento_id_real'])) {
