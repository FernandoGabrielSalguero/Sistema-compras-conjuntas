...
document.addEventListener('DOMContentLoaded', () => {
    // Delegación de eventos para todos los data-action
    document.addEventListener('click', (e) => {
        ...
        switch(action) {
            ...
            case 'showAlert':
                showAlert(target.dataset.type, target.dataset.message);
                break;

            case 'showToast':
                showToast(target.dataset.type, target.dataset.message);
                break;

            case 'showToastBoton':
                showToastBoton(target.dataset.type, target.dataset.message);
                break;

            case 'toggleRoundMetric':
                toggleRoundMetric(target);
                break;
            ...
            case 'toggleSubcategorias':
                toggleSubcategorias(target);
                break;
...
function showToastBoton(type = 'info', message = 'Mensaje') {
    ...
    toast.innerHTML = `
        <button class="toast-close material-icons" data-action="closeToast" aria-label="Cerrar notificación">
...
function showTourStep(index) {
    ...
    // Crear elementos de forma segura con createElement
    const p = document.createElement('p');
    p.textContent = step.message; // textContent es seguro
    ...
    if (index > 0) {
        ...
        prevBtn.setAttribute('data-action', 'prevTourStep');
        actions.appendChild(prevBtn);
    }

    const nextBtn = document.createElement('button');
    nextBtn.textContent = index < tourSteps.length - 1 ? 'Siguiente' : 'Finalizar';
    nextBtn.setAttribute('data-action', index < tourSteps.length - 1 ? 'nextTourStep' : 'endTour');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderList(items) {
        ...
        items.forEach((it, i) => {
            ...
            // Usar escapeHtml para sanitizar
            li.innerHTML = `
        <span class="material-icons" aria-hidden="true">person</span>
        <div>
          <strong>${escapeHtml(it.nombre)}</strong><br>
          <small>${escapeHtml(it.doc ?? '')}</small>
        </div>
      `;
...
/* ============================================
   FUNCIONES NO USADAS EN EL PROYECTO ACTUAL
   (Comentadas para referencia futura)
   ============================================ */

// ===== SELECT PRO =====
// Funciones para un componente de select personalizado con búsqueda
// No están en uso actualmente en el HTML del proyecto
...
// Funciones de validación para inputs específicos por ID
// No están siendo llamadas desde el HTML actual
