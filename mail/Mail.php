...
final class Mail
{
    ...
    /**
     * Envia correo por modificacion de solicitud de drones a productor, cooperativas y correos fijos.
     * @return array{ok:bool, error?:string}
     */
    public static function enviarSolicitudDronActualizadaCooperativa(array $data): array
    {
        try {
            $tplPath = __DIR__ . '/template/drone_modificacionPedido_cooperativa.html';
            ...
            $content = sprintf(
                '<h2 style="margin:0 0 6px 0;">Solicitud de drones modificada</h2>
            ...
            $subject = 'Se modifico la solicitud de drones N° ' . $sid;
            ...
            $meta = [
                'contrato_id' => $sid,
                'correo' => $prodCorreo,
                'enviado_por' => 'coop_update',
            ];

            $ok = self::sendAndLog($mail, 'dron_modificacion', 'drone_modificacionPedido_cooperativa.html', $meta);
            return ['ok' => (bool)$ok];
        } catch (\Throwable $e) {
            return ['ok' => false, 'error' => $e->getMessage()];
        }
