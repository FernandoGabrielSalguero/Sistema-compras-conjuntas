...
final class DroneVariableModel
{
    ...
    private function tableFor(string $entity): string
    {
        switch ($entity) {
            ...
            case 'pilotos':        return 'dron_pilotos';
    ...
    // ------- create/update (genérico y pilotos)
    ...
    public function createPiloto(string $nombre, ?string $telefono, ?string $zona, string $correo): int
    {
        $tbl = $this->tableFor('pilotos');
        $st = $this->pdo->prepare("INSERT INTO {$tbl} (nombre, telefono, zona_asignada, correo) VALUES (:n, :t, :z, :c)");
        $st->execute([':n'=>$nombre, ':t'=>$telefono, ':z'=>$zona, ':c'=>$correo]);
        return (int)$this->pdo->lastInsertId();
    }

    public function updatePiloto(int $id, string $nombre, ?string $telefono, ?string $zona, string $correo): bool
    {
        $tbl = $this->tableFor('pilotos');
        $st = $this->pdo->prepare("UPDATE {$tbl} SET nombre = :n, telefono = :t, zona_asignada = :z, correo = :c WHERE id = :id");
        return $st->execute([':n'=>$nombre, ':t'=>$telefono, ':z'=>$zona, ':c'=>$correo, ':id'=>$id]);
    }
