<?php
// MODEL del drawer: detalle FULL + actualización + catálogos
...
final class DroneDrawerListadoModel
{
    ...
    private function getCostoBaseHaPorDefecto(): ?float
    {
        $row = $this->pdo->query("SELECT costo FROM dron_costo_hectarea LIMIT 1")->fetch();
    ...
    ");
        ...
        // --- costos (pueden no existir)
        $st = $this->pdo->prepare("SELECT * FROM drones_solicitud_costos WHERE solicitud_id = :id LIMIT 1");
        $st->execute([':id' => $id]);
    ...
    ");
        ...
        // ===== costos efectivos (persistidos o calculados) =====
        $costosEfectivos = $costos; // si hay fila, usarla tal cual
    ...
    {
        return $this->pdo->query("SELECT id, nombre FROM dron_formas_pago WHERE activo='si' ORDER BY nombre")->fetchAll() ?: [];
    }
    public function listPatologias(): array
    {
        return $this->pdo->query("SELECT id, nombre FROM dron_patologias WHERE activo='si' ORDER BY nombre")->fetchAll() ?: [];
    }
    public function listProductos(): array
    {
        return $this->pdo->query("SELECT id, nombre, costo_hectarea FROM dron_productos_stock WHERE activo='si' ORDER BY nombre")->fetchAll() ?: [];
    }
    public function listCooperativas(): array
    {
        $sql = "SELECT id_real, usuario FROM usuarios WHERE rol='cooperativa' AND permiso_ingreso='Habilitado' ORDER BY usuario";
        return $this->pdo->query($sql)->fetchAll() ?: [];
    }
