...
<style>
  ...
  .sv-drawer__panel{ position:absolute; top:0; right:0; height:100%; width:min(760px,100%); background:#fff; box-shadow:-6px 0 24px #00000022; display:flex; flex-direction:column; border-top-left-radius:16px; border-bottom-left-radius:16px; }
  ...
  .sv-drawer__body{ flex:1; overflow:auto; -webkit-overflow-scrolling:touch; padding:16px 20px; background-color:darkgray; }
  ...
  .data-table td .input-icon{ display:block; }
  .data-table td .input-icon input,
  .data-table td .input-icon select,
  .data-table td .input-icon textarea { width:100%; }

  .card .form-separator{ margin:8px 0 16px; }

  .chips { display:flex; gap:8px; flex-wrap:wrap; }
  .chip-pill{ display:inline-flex; align-items:center; gap:6px; background:#eef; border-radius:999px; padding:4px 10px; }
  .chip-pill .close{ cursor:pointer; border:0; background:transparent; font-weight:bold; }

  .producto-item{ border:1px solid #eee; border-radius:12px; padding:10px; margin-bottom:8px; display:grid; grid-template-columns:1fr auto; gap:6px; }
...
(function () {
  ...
  const state   = { motivos:[], rangos:[], items:[] }; // items incluyen receta única por producto
  ...
  function renderProductos(){
    ...
    state.items.forEach((it,idx)=>{
      ...
      wrapper.querySelector('[data-role="quitar"]').addEventListener('click', ()=>{
        state.items.splice(idx,1); renderProductos(); renderRecetaCombinada(); recalcCostos();
      });
  ...
  // utils set/get
  function setV(id,val){ const n=document.getElementById(id); if(!n)return; if(n.type==='checkbox') n.checked=Boolean(val); else n.value=(val??'')===null?'':String(val??''); }
  function getV(id){ const n=document.getElementById(id); if(!n)return null; return n.value===''? null : n.value; }
  ...
  $('#btn_add_patologia')?.addEventListener('click', ()=>{
    ...
    if (val === '__otra__'){
      ...
      state.motivos.push({ patologia_id:null, es_otros:1, otros_text:txt });
    } else {
      state.motivos.push({ patologia_id:Number(val), es_otros:0, otros_text:'' });
    }

    $('#patologia_new').value=''; $('#patologia_new_text').value='';
  ...
  $('#btn_add_producto')?.addEventListener('click', ()=>{
    ...
    state.items.push({
      ...
      receta: { principio_activo:null, dosis:null, unidad:'', orden_mezcla:null, notas:'' }
    });

    $('#producto_new').value=''; $('#producto_new_patologia').value='';
    renderProductos(); renderRecetaCombinada(); recalcCostos();
  });
