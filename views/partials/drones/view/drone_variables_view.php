...
<div class="content">
  ...
  <!-- Pilotos de drones -->
  <div id="card-pilotos" class="card tabla-card" aria-labelledby="h-pilotos">
    <h4 id="h-pilotos">Pilotos de drones</h4>

    <form id="form-pilotos" class="form-grid grid-3" autocomplete="off" aria-describedby="pl-msg">
      <input type="hidden" name="id" id="pl-id" value="">
      <div class="input-group">
        <label for="pl-nombre">Nombre</label>
        <div class="input-icon input-icon-name">
          <input type="text" id="pl-nombre" name="nombre" placeholder="Nombre y apellido" required maxlength="100" />
        </div>
      </div>
      <div class="input-group">
        <label for="pl-telefono">Teléfono</label>
        <div class="input-icon input-icon-phone">
          <input type="tel" id="pl-telefono" name="telefono" placeholder="Ej.: +54 9 387 555-1234" maxlength="20" />
        </div>
      </div>
      <div class="input-group">
        <label for="pl-zona">Zona asignada</label>
        <div class="input-icon input-icon-location">
          <input type="text" id="pl-zona" name="zona_asignada" placeholder="Ej.: Valle de Lerma" maxlength="100" />
        </div>
      </div>
      <div class="input-group">
        <label for="pl-correo">Correo</label>
        <div class="input-icon input-icon-email">
          <input type="email" id="pl-correo" name="correo" placeholder="piloto@correo.com" required maxlength="100" />
        </div>
      </div>

      <div class="form-grid grid-3">
        <button id="pl-submit" type="submit" class="btn btn-aceptar">Guardar</button>
        <button id="pl-cancel" type="button" class="btn btn-cancelar" aria-label="Cancelar edición">Cancelar</button>
        <button id="pl-limpiar" type="button" class="btn btn-info">Limpiar</button>
      </div>
    </form>
    ...
    <div class="form-grid grid-3" style="margin-top:8px;">
      <div class="input-group">
        ...
        <div class="input-icon input-icon-search">
          <input type="text" id="pl-q" placeholder="Filtrar por nombre/teléfono/zona/correo" />
        </div>
      </div>
      <div class="input-group">
        <label for="pl-inactivos">Ver inactivos</label>
        <label class="switch">
          <input type="checkbox" id="pl-inactivos" />
          <span class="slider"></span>
        </label>
      </div>
    </div>

    <div id="pl-msg" role="status" aria-live="polite" class="muted" style="margin:6px 0;"></div>

    <div class="tabla-wrapper" style="margin-top:8px;">
      <table class="data-table" aria-describedby="h-pilotos">
        <thead>
          <tr>
            <th>#</th>
            <th>Nombre</th>
...
(function() {
  ...
  function renderRow(item, tbody, entity) {
    ...
    if (entity === 'pilotos') {
      tr.append(
        ...
        el('td', { textContent: item.telefono || '' }),
        el('td', { textContent: item.zona_asignada || '' }),
        el('td', { textContent: item.correo || '' })
      );
    } else {
      tr.append(
        el('td', { textContent: item.id }),
    ...
    const btnToggle = el('button', { className: 'btn ' + (estadoOk ? 'btn-cancelar' : 'btn-aceptar'), type: 'button', textContent: estadoOk ? 'Eliminar' : 'Reactivar', 'aria-label': estadoOk ? 'Eliminar' : 'Reactivar' });
    ...
    btnEdit.addEventListener('click', () => {
      if (entity === 'patologias') {
        ...
        $('#fp-nombre').focus();
      } else if (entity === 'pilotos') {
        $('#pl-id').value = item.id;
        $('#pl-nombre').value = item.nombre || '';
        $('#pl-telefono').value = item.telefono || '';
        $('#pl-zona').value = item.zona_asignada || '';
        $('#pl-correo').value = item.correo || '';
        $('#pl-nombre').focus();
      }
    });
    ...
    btnToggle.addEventListener('click', async () => {
      ...
      if (res.ok) {
        ...
        if (entity === 'patologias') await loadPatologias();
        else if (entity === 'produccion') await loadProduccion();
        else if (entity === 'formas_pago') await loadFormasPago();
        else if (entity === 'pilotos') await loadPilotos();
      } else {
  ...
  document.getElementById('form-patologias').addEventListener('submit', async (e) => {
    e.preventDefault();
  ...
  document.getElementById('p-cancel').addEventListener('click', () => {
    $('#p-id').value = ''; $('#p-nombre').value = ''; $('#p-desc').value = ''; $('#p-msg').textContent = '';
    show('info', 'Edición cancelada');
  });
  document.getElementById('p-limpiar').addEventListener('click', () => {
    $('#p-q').value = ''; $('#p-inactivos').checked = false; show('info', 'Filtros limpiados'); loadPatologias();
  });
  document.getElementById('p-q').addEventListener('input', debounce(loadPatologias, 300));
  document.getElementById('p-inactivos').addEventListener('change', loadPatologias);
  ...
  document.getElementById('r-cancel').addEventListener('click', () => {
    $('#r-id').value = ''; $('#r-nombre').value = ''; $('#r-desc').value = ''; $('#r-msg').textContent = '';
    show('info', 'Edición cancelada');
  });
  document.getElementById('r-limpiar').addEventListener('click', () => {
    $('#r-q').value = ''; $('#r-inactivos').checked = false; show('info', 'Filtros limpiados'); loadProduccion();
  });
  document.getElementById('r-q').addEventListener('input', debounce(loadProduccion, 300));
  document.getElementById('r-inactivos').addEventListener('change', loadProduccion);

  // ---------- Pilotos ----------
  const loadPilotos = async () => {
    const q = $('#pl-q').value.trim();
    const ina = $('#pl-inactivos').checked;
    const tbody = $('#pl-tbody');
    tbody.innerHTML = '';
    const data = await list('pilotos', q, ina);
    if (!data.ok) {
      $('#pl-msg').textContent = 'Error: ' + (data.error || 'No se pudo cargar');
      show('error', 'No se pudo cargar Pilotos');
      return;
    }
    (data.data || []).forEach(it => renderRow(it, tbody, 'pilotos'));
    $('#pl-msg').textContent = (data.data || []).length ? '' : 'Sin resultados.';
    if (!(data.data || []).length) show('info', 'Sin resultados en Pilotos');
  };

  document.getElementById('form-pilotos').addEventListener('submit', async (e) => {
    e.preventDefault();
    const id = parseInt($('#pl-id').value || '0', 10) || null;
    const nombre = $('#pl-nombre').value.trim();
    const telefono = $('#pl-telefono').value.trim();
    const zona_asignada = $('#pl-zona').value.trim();
    const correo = $('#pl-correo').value.trim();
    const res = await save('pilotos', { id, nombre, telefono, zona_asignada, correo });
    if (res.ok) {
      show('success', id ? 'Piloto actualizado' : 'Piloto creado');
      $('#pl-id').value = ''; $('#pl-nombre').value = ''; $('#pl-telefono').value = ''; $('#pl-zona').value = ''; $('#pl-correo').value = '';
      await loadPilotos();
    } else {
      show('error', res.error || 'No se pudo guardar');
    }
  ...
  document.getElementById('pl-cancel').addEventListener('click', () => {
    $('#pl-id').value = ''; $('#pl-nombre').value = ''; $('#pl-telefono').value = ''; $('#pl-zona').value = ''; $('#pl-correo').value = '';
    $('#pl-msg').textContent = ''; show('info', 'Edición cancelada');
  });
  document.getElementById('pl-limpiar').addEventListener('click', () => {
    $('#pl-q').value = ''; $('#pl-inactivos').checked = false; show('info', 'Filtros limpiados'); loadPilotos();
  });
  document.getElementById('pl-q').addEventListener('input', debounce(loadPilotos, 300));
  document.getElementById('pl-inactivos').addEventListener('change', loadPilotos);
  ...
  document.getElementById('form-costo').addEventListener('submit', async (e) => {
    ...
    if (!isFinite(costo) || costo < 0) { show('error', 'Ingresá un costo válido (>= 0)'); return; }
    ...
    if (res.ok) { show('success', 'Costo actualizado'); await loadCosto(); }
    else { show('error', res.error || 'No se pudo actualizar el costo'); }
  });
  document.getElementById('c-cancel').addEventListener('click', async () => { await loadCosto(); show('info', 'Cambios descartados'); });
  ...
  document.getElementById('form-formaspago').addEventListener('submit', async (e) => {
    ...
    const res = await save('formas_pago', { id, nombre, descripcion });
    if (res.ok) {
      show('success', id ? 'Forma de pago actualizada' : 'Forma de pago creada');
      $('#fp-id').value = ''; $('#fp-nombre').value = ''; $('#fp-desc').value = '';
  ...
  document.getElementById('fp-cancel').addEventListener('click', () => {
    $('#fp-id').value = ''; $('#fp-nombre').value = ''; $('#fp-desc').value = '';
    $('#fp-msg').textContent = ''; show('info', 'Edición cancelada');
  });
  document.getElementById('fp-limpiar').addEventListener('click', () => {
    $('#fp-q').value = ''; $('#fp-inactivos').checked = false; show('info', 'Filtros limpiados'); loadFormasPago();
  });
  document.getElementById('fp-q').addEventListener('input', debounce(loadFormasPago, 300));
  document.getElementById('fp-inactivos').addEventListener('change', loadFormasPago);
