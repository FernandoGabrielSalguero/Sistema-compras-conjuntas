...
<div class="content">
  ...
  <!-- Pilotos de drones (solo lectura desde usuarios/usuarios_info) -->
  <div id="card-pilotos" class="card tabla-card" aria-labelledby="h-pilotos">
    ...
    <div class="form-grid grid-3" style="margin-top:8px;">
      <div class="input-group">
        ...
        <div class="input-icon input-icon-search">
          <input type="text" id="pl-q" placeholder="Filtrar por nombre/usuario/teléfono/zona/correo" />
        </div>
      </div>
      <div class="input-group">
    ...
    function renderRow(item, tbody, entity) {
      ...
      if (entity === 'pilotos') {
        tr.append(
          ...
          el('td', {
            textContent: item.telefono || ''
          }),
          el('td', {
            textContent: item.zona_asignada || ''
          }),
          el('td', {
            textContent: item.correo || ''
          })
        );
      } else {
        tr.append(
      ...
      // Acciones SOLO para entidades editables (no para pilotos)
      if (entity !== 'pilotos') {
        const tdAcc = el('td');
        const wrap = el('div', {
