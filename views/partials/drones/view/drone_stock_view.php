  ...
  <div class="card" id="stock-form-card" aria-labelledby="form-title">
    ...
    <form id="producto-form" autocomplete="off" novalidate>
      ...
      <div class="form-grid grid-3">
        ...
        <div class="input-group">
          <label for="costo_hectarea">Costo por hectárea</label>
          <div class="input-icon input-icon-money">
            <input type="number" id="costo_hectarea" name="costo_hectarea" placeholder="0.00" step="0.01" min="0" required aria-required="true" />
          </div>
        </div>
...
<script>
  (function() {
    ...
    $('#producto-form').addEventListener('submit', async (e) => {
      ...
      if (payload.cantidad_deposito < 0) {
        showAlert('error', 'La cantidad no puede ser negativa.');
        return;
      }
      if (payload.costo_hectarea < 0) {
